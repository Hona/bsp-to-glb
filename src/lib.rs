...
pub fn export_bsp(data: &[u8]) -> Result<ExportResult, String> {
    export_bsp_with_options(data, &ExportOptions::default())
}
...
pub fn export_bsp_with_options(
    ...
    let lightmaps = extract_lightmaps(
        &bsp,
        selected_lightmaps,
        &faces,
        &texinfos,
    ...
    for (model_index, model) in models.iter().enumerate() {
        ...
        for (face_index, face) in faces.iter().copied().enumerate().take(end).skip(start) {
                ...
                .zip(&source_normals)
            {
                ...
                if let (Some(extracted), Some(placement)) = (&lightmaps, mapping) {
    ...
    Ok(ExportResult {
        glb,
        stats,
        lightmaps: lightmaps.map(|extracted| extracted.artifacts),
    })
}
